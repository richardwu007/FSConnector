...
class BrowserView:
    ...
    class BrowserForm(WinForms.Form):
        def __init__(self, window, cache_dir):
            ...
            self.real_url = None
            self.Text = window.title #'hack winforms title' #
            ...
            ico_path = os.path.join(base_path, 'Fastsuite_E2_Icon.ico')
            self.Icon = Icon(ico_path)
            # hack top bar color
            self.FormBorderStyle = WinForms.FormBorderStyle.Fixed3D
            #self.TitleBar.ForeColor = ColorTranslator.FromHtml('#2c3e50')
